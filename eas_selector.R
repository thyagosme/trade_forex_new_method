...
run_model <- function(df,
  ...
  for (i in 3:(length(periods) - 1)) {
    ...
    # Classes previstas
    # -------------------------------------------------------
    Pred_train_class <- as.integer(Pred_train_prob >= 0.5)
    Pred_val_class   <- as.integer(Pred_val_prob >= 0.5)
...
for (b in 1:B) {
  ...
  Profit[b] <- sum(out$summary_results$sum_profit_best_test)
  N[b] <- sum(out$summary_results$passed_val_filter)
  # all_runs[[b]] <- out
}

length(out$results_list)
...
p_pos <- sum(Profit > 0) / sum(Profit != 0)
print(p_pos)

N
mean(N)
mean(N) / length(out$results_list)

ic <- round(c(m - 1.96 * s / sqrt(B), m + 1.96 * s / sqrt(B)), 1)
print(ic)
